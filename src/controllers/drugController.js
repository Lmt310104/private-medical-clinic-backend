      ...
      if (req.isAuthenticated()) {
        try {
          const authorization = await db.authorizations.findOne({
            where: { userGroupId: req.user.user.roleId, featId: 22 },
          });
...
const addDrug = asyncHandler(async (req, res) => {
  ...
  try {
    if (req.isAuthenticated()) {
      try {
        const authorization = await db.authorizations.findOne({
          where: { userGroupId: req.user.user.roleId, featId: 23 },
        });
        if (!authorization.isAccess) {
          return res.status(401).json({
            status: res.statusCode,
...
const updateDrug = asyncHandler(async (req, res) => {
  try {
    if (req.isAuthenticated()) {
      try {
        const authorization = await db.authorizations.findOne({
          where: { userGroupId: req.user.user.roleId, featId: 24 },
        });
        if (!authorization.isAccess) {
          return res.status(401).json({
            status: res.statusCode,
...
const deleteDrug = asyncHandler(async (req, res) => {
  try {
    if (req.isAuthenticated()) {
      try {
        const authorization = await db.authorizations.findOne({
          where: { userGroupId: req.user.user.roleId, featId: 25 },
        });
        if (!authorization.isAccess) {
          return res.status(401).json({
            status: res.statusCode,
